...
export function StudentTimetable({ studentMatric }: Props) {
  const [timetable] = useState<TimetableSlot[]>(getLocalTimetable());
  const registrations = getLocalRegistrations().filter(
    (r) => r.studentMatric === studentMatric && r.semester === SEMESTER,
  );
  const myCourseCodes = new Set(registrations.map((r) => r.courseCode));

  const mySlots = timetable.filter(
    (s) => myCourseCodes.has(s.courseCode) && s.semester === SEMESTER,
  );
  ...
  const hasSomeSlots = mySlots.length > 0;
    ...
    <div className="space-y-6">
      ...
      {!hasSomeSlots && (
        <Card>
          ...
          >
            No timetable slots found. Register for courses to see your schedule.
          </CardContent>
        </Card>
      )}
      ...
      <div className="space-y-4">
        {DAYS.map((day) => (
          <Card
            key={day}
            className={`border ${slotsByDay[day].length ? dayColors[day] : ""}`}
          >
              ...
              {slotsByDay[day].length === 0 ? (
                ...
                <div className="space-y-2">
                  {slotsByDay[day].map((slot, i) => (
                    <div
                      key={slot.id}
                      className="bg-white rounded-lg border border-slate-200 p-3 flex items-start justify-between"
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">
                          {slot.courseCode} &bull; {slot.lecturerName}
                        </p>
                      ...
                      <Badge
                        className={`${dayBadgeColors[day]} border-0 text-xs flex-shrink-0`}
                      >
                        {day}
                      </Badge>
                    </div>
